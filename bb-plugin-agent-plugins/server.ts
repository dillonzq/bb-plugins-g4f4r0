...
import { oauthRedirectBase, serverAppUrl } from "./src/oauth-redirect.js";
...
export default async function plugin(bb: BbPluginApi) {
  ...
  const oauthSettings = bb.settings.define({
    ...
    oauthRedirectBaseUrl: {
      ...
      description: "Public origin for MCP OAuth callbacks when the browser is not on this server. Example: https://g4f4r0.getbb.app. Empty uses BB_APP_URL, then loopback.",
      default: "",
    },
  });
    ...
    oauth: {
      async getProvider(pluginId, serverId, serverUrl) {
        ...
        const base = oauthRedirectBase({
          setting: settings.oauthRedirectBaseUrl,
