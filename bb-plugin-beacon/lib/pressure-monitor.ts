...
export function createPressureMonitor(bb: BbPluginApi, read: (signal: AbortSignal) => Promise<LightSample>, reset: () => void) {
  ...
  function configure(nextEnabled: boolean, nextNotifications: boolean) {
    ...
    if (!enabled && store) {
      try { store.write([{ type: "monitor_disabled", timestamp: Date.now() }], freshPressureState()); }
  ...
  async function tick(signal: AbortSignal) {
    ...
    // Persist the two metric states alongside summaries/transitions, not every refresh.
    const saved = records.length ? storage.write(records, result.state) : [];
    state = result.state;
    ...
    try {
      while (!signal.aborted && !disposed) {
        ...
        try { await tick(runController.signal); }
          ...
          if (state) state = stepPressure(state, { cpu: null, memory: null }, Date.now()).state;
