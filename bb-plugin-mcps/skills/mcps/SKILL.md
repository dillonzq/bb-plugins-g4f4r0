...
3. `mcps_call` — invoke that `opaqueId` using the card as the argument template. For write/destructive tools, ask the user and pass `confirm=true`. Repeat the returned text in your reply so the user can see it.
