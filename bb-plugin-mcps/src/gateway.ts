 ...
 *
 * The protocol and transports are delegated to the official MCP client SDK.
 * This class owns BB concerns: approval/enabled gates, process containment,
 * catalog namespacing, durable lifecycle state, and one-tool lookup.
 */
...
export interface McpGatewayOptions {
  ...
  /** How long search waits for cold catalogs before returning what is ready. */
  searchWaitMs?: number;
}
...
export function isMcpRequest(url: URL, configuredUrl: URL, input: RequestInfo | URL, init: RequestInit | undefined): boolean {
  // Literal mcp.json headers never leave the configured origin. Cross-origin
  // JSON-RPC POSTs and event-stream GETs used to inherit them.
