...
let videoMode=true,rtcPeer=null,rtcVideo=null,rtcTimeout=null,rtcStatsTimer=null,rtcKeepalive=null;
...
function stopVideo(){clearTimeout(rtcTimeout);clearInterval(rtcStatsTimer);clearInterval(rtcKeepalive);rtcPeer?.close();rtcVideo?.remove();rtcPeer=null;rtcVideo=null;}
function openVideo(){
 ...
 rtcKeepalive=setInterval(()=>{if(ws.readyState===1)ws.send('BROWSE_PING');},2000);
 ...
 ws.onclose=e=>{rtcReport('signal-close',{code:e.code,clean:e.wasClean});if(!closed&&pc.connectionState!=='closed')rtcFail('Connection to the test server closed.');else stopVideo();};
}
`; }
