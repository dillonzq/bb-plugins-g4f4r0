...
const root=process.argv[2];if(!root)throw Error('host-data path required');
...
try {
 ...
 await cdp.send('Page.navigate',{url:'http://127.0.0.1:38886/api/v1/plugins/browse/http/viewer?id=linkcheck'});
 let result:any;
 for(let i=0;i<50;i++){
...
} finally {cdp?.close();await browser.close();await fs.rm(browser.profile,{recursive:true,force:true});}
