...
Post-deployment lifecycle validation started three simultaneous managed video sessions in this thread. Their cold starts completed in 2.35–3.38 seconds; a fourth start was rejected at the configured 3/3 limit with reuse/cleanup guidance. Release followed by reconnect initially exposed a race where the video browser's first page had not appeared when CDP attached. Browse now waits up to five seconds for that managed video target while retaining strict immediate tab checks for native sessions. A deployed retry reconnected the same profile successfully in 2.06 seconds. The final suite passed 185 tests across 34 files, and the fixture server, sessions, browser processes, and disposable profiles were removed.
