...
const out='/tmp/dusk-qa';await mkdir(out,{recursive:true});
...
try {
 ...
 await page.locator('"'.dusk-customize:not([disabled])').waitFor();
 ...
 await page.waitForFunction(()=>document.querySelector('.dusk-customize')?.getAttribute('aria-busy')==='true');
 assert(await page.locator('.dusk-customize').isDisabled());
 assert.equal(await page.locator('.dusk-customize .animate-spin').count(),1);
 ...
 for(const [width,height] of sizes) {
  ...
  const state=await page.evaluate(()=>{
   const host=document.querySelector('.dusk-home'), shell=document.querySelector('[data-promptbox-shell]'),c=document.querySelector('.dusk-wallpaper'),b=document.querySelector('.dusk-customize');
   const h=host.getBoundingClientRect(),s=shell.getBoundingClientRect(),r=b.getBoundingClientRect();
   return {overflow:document.documentElement.scrollWidth>innerWidth,center:[(s.left+s.right-h.left-h.right)/2,(s.top+s.bottom-h.top-h.bottom)/2],shell:[s.top,s.bottom],buttonVisible:document.elementFromPoint(r.x+r.width/2,r.y+r.height/2)?.closest('button')===b,ratioError:Math.abs(c.width/c.height-c.clientWidth/c.clientHeight),canvasCount:document.querySelectorAll('.dusk-wallpaper').length};
  });
 ...
 await page.waitForFunction(()=>document.querySelector('.dusk-customize')?.getAttribute('aria-busy')==='true');
 await page.waitForFunction(()=>document.querySelector('.dusk-customize')?.getAttribute('aria-busy')==='false');
 assert(config.image);assert(await page.locator('.dusk-customize').isEnabled());
 ...
 assert(await page.locator('.dusk-customize').isEnabled());
 if(mobile) {
