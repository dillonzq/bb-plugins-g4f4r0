...
- `server.ts` — the backend: a todo store in `bb.storage.kv`, RPC methods
  for the page, a `bb sidekick` CLI command, a setting, and a realtime signal
...
- `skills/example-todos/SKILL.md` — a skill that tells agents how to keep the list
  with `bb sidekick`. BB imports it into agent threads automatically.
...
`bb sidekick add "Ship it"` in a terminal. The page updates at once.
...
```
bb plugin config sidekick
bb plugin config sidekick set showDone false
bb plugin reload sidekick
```
